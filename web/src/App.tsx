...
const INTERNATIONAL_LEAGUES: { key: string; name: string; flag: string }[] = [
  { key: 'NationsLeague', name: 'UEFA Nations League', flag: '🇪🇺' },
  { key: 'WorldCup', name: 'FIFA World Cup', flag: '🏆' },
  { key: 'WCQ_UEFA', name: 'FIFA World Cup Qualifiers - UEFA', flag: '🇪🇺' },
  { key: 'WCQ_CONMEBOL', name: 'FIFA World Cup Qualifiers - CONMEBOL', flag: '🌎' },
  { key: 'WCQ_CAF', name: 'FIFA World Cup Qualifiers - CAF', flag: '🌍' },
  { key: 'Euro', name: 'UEFA European Championship', flag: '🇪🇺' },
  { key: 'CopaAmerica', name: 'Copa América', flag: '🌎' },
  { key: 'AFCON', name: 'Africa Cup of Nations', flag: '🌍' },
  { key: 'Friendlies', name: 'International Friendlies', flag: '🤝' },
  { key: 'GoldCup', name: 'CONCACAF Gold Cup', flag: '🏆' },
];

  // Compute available football leagues (including international competitions)
  const footballLeagues = useMemo(() => {
    ...
    data.football.upcoming.forEach((m) => {
      if (m.league && !map.has(m.league)) {
        map.set(m.league, { key: m.league, name: m.league_name || m.league, flag: m.flag || '⚽' });
      }
    });

    // 2. International competitions
    INTERNATIONAL_LEAGUES.forEach((intl) => {
      if (!map.has(intl.key)) {
        map.set(intl.key, intl);
      }
    });

    // 3. Historical tracker entries
    data.football.tracker.forEach((t) => {
      if (t.league && !map.has(t.league)) {
        map.set(t.league, { key: t.league, name: t.league, flag: '⚽' });
      }
    });
            ...
            <div className="bg-dark-800/80 border border-dark-700 rounded-xl p-4 shadow-sm space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  >
                    <option value="ALL">{sport === 'football' ? `🌍 All Competitions (${footballLeagues.length} Leagues)` : '🎾 All Circuits (ATP & WTA)'}</option>
                    {sport === 'football'
              ...
              <div className="space-y-4">
                {filteredFootball.length === 0 ? (
                  ...
                  filteredFootball.map((m) => (
                    <ErrorBoundary key={m.match_id} fallbackTitle={`Football Fixture Error (${m.home_team} vs ${m.away_team})`}>
