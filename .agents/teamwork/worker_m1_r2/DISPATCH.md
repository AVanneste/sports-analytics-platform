...
1. Update Football/football_core/utils/helpers.py:
   - Make normalize_team_name case-insensitive and accent-insensitive using _LOWER_TEAM_NAME_MAP. Add 'Bosnia' -> 'Bosnia and Herzegovina'.
   - Fix teams_match to eliminate false positive collisions on sovereign nations and distinct clubs (Niger vs Nigeria, South Korea vs North Korea, Republic of Ireland vs Northern Ireland, Congo vs DR Congo, Sudan vs South Sudan, Guinea vs Guinea-Bissau/Equatorial Guinea, Dominica vs Dominican Republic, Manchester City vs Manchester United). Implement the token-based directional/qualifier guards and expanded stop words specified in explorer_m1_r2_helpers/handoff.md.
...
4. Update tests:
   - In tests/test_adversarial_m1.py, update tests to reflect the safe empty competition return value `[]` and verify inf/nan rejection.
   - In tests/test_milestone1_adversarial.py, update assertions in TestAdversarialFindings to verify the remediated behavior (asserting 0 collisions, successful normalization of "Bosnia", and True for lowercase aliases).
...
6. Write your handoff report to /home/antoine/Code/AG_sports_data/.agents/teamwork/worker_m1_r2/handoff.md and send a message to parent.
